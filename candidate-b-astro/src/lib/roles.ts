...
export type RoleValidation =
  //type это второй способ объявить тип и в отличие от interface, он может описывать не только объекты.
...
function isPlainObject(value: unknown): value is Record<string, unknown> {
  /* из-за того , что здесь есть type predicate, а именно - value 
...
export function validateRole(input: unknown, index: number): RoleValidation {
  ...
  if (!isNonEmptyString(slug)) {
    return {
      ok: false,
      error: `role #${index}: "slug" must be a non-empty string`,
    };
  }
  if (!isNonEmptyString(title)) {
    return {
      ok: false,
      error: `role "${slug}": "title" must be a non-empty string`,
    };
  }
  if (!isNonEmptyString(department)) {
    return {
      ok: false,
      error: `role "${slug}": "department" must be a non-empty string`,
    };
  }
  if (!isNonEmptyString(location)) {
    return {
      ok: false,
      error: `role "${slug}": "location" must be a non-empty string`,
    };
  }
